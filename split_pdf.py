...
from tkinter import Tk, Label, TOP, Button, Checkbutton, StringVar, BooleanVar, Entry, mainloop
...
class ImGUI:
	def __init__(self):
		...
		self.one_file_cb = Checkbutton(self.root, text="Save all pages in one file", variable=self.one_file,
										command=self.toggle_one_file)
		self.one_file_cb.pack(side=TOP, pady=10)
	...
	def toggle_one_file(self):
		self.one_file = not self.one_file
	...
	def cutme(self):
		try:
			...
			# print(pages)
			if self.one_file:
				self.cut_in_one(mypdf, pages)
			else:
				self.cut_many(mypdf, pages)
			messagebox.showinfo('Done', "Files cut successfully")

	def cut_in_one(self, mypdf, pages):
		print('Cut all in one page')
		# Save all sets of pages in one file
		name = f'{self.saveaddress}\\{self.name}.pdf'
		print('MYNAME', name)
	...
	def cut_many(self, mypdf, pages):
		counter = 1
		for pair in pages:
			name = f'{self.saveaddress}\\{self.name}_part_{counter}.pdf' if len(pages) > 1 else \
					f'{self.saveaddress}\\{self.name}.pdf'
			print('MYNAME', name)
			my_file = open(name, "wb")
			writer = PdfWriter()
